                    ...
                    <form id="bannerForm" action="index.php?controller=admin&action=editBanner&id=<?php echo $banner['id']; ?>" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select border-primary" id="position" name="position" required>
                                <option value="hero_slider" <?php echo (($banner['position'] ?? '') == 'hero_slider') ? 'selected' : ''; ?>>Hero Slider (Ảnh chính trên cùng - nhiều slide)</option>
                                <option value="exclusive" <?php echo (($banner['position'] ?? '') == 'exclusive') ? 'selected' : ''; ?>>Phân Phối Độc Quyền (4 banner dọc)</option>
                                <option value="toptrend" <?php echo (($banner['position'] ?? '') == 'toptrend') ? 'selected' : ''; ?>>Top Trend Hôm Nay (4 Banner dọc)</option>
                                <option value="promo_row" <?php echo (($banner['position'] ?? '') == 'promo_row') ? 'selected' : ''; ?>>Khuyến Mãi Ngang (3 Banner ngang)</option>
